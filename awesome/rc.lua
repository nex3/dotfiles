...
mytextclock = awful.widget.textclock({ align = "right" }, " <span weight='bold' color='#fff'>%l:%M</span> %a %d %b ")
    ...
    globalkeys = awful.util.table.join(globalkeys,
        ...
        awful.key({ mod, "Control" }, char,
                  function ()
                      local screen = mouse.screen
                      if tagfn(screen) then
                          awful.tag.viewtoggle(tagfn(screen))
                      end
                  end),
...
for i = 1, keynumber do
    tagkey(modkey, "#" .. i + 9, function(screen) return tags[screen][i] end)
end
